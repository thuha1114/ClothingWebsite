...
export const Follow_Products_Men = () => {
    ...
    const list = useMemo(()=>{
        ...
        const results = all_products.filter(item => item.category === 'men').slice(random,random + 4)
        return results
    },[])
    ...
    return (
        <div className='mb-24'>
          ...
          <div className='grid grid-cols-4 gap-8 mx-32'>
            {list.map(item => (
              <a href='https://www.instagram.com/selectedindia/' key= {item.id}>
